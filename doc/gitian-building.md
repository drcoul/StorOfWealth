...
*Setup instructions for a gitian build of Store of Wealth Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Store of Wealth Coin
...
- [Building Store of Wealth Coin](#building-storofwealth)
...
Clone the git repositories for storofwealth and gitian and then checkout the storofwealth version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/storofwealth-crypto/storofwealth.git
cd storofwealth
...
Follow the instructions in [doc/release-process.md](release-process.md) in the storofwealth repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Store of Wealth Coin
----------------

To build Store of Wealth Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the storofwealth repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/storofwealth/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/storofwealth-crypto/storofwealth
...
For example:
```bash
URL=https://github.com/crowning-/storofwealth.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit storofwealth=${COMMIT} --url storofwealth=${URL} ../storofwealth/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit storofwealth=${COMMIT} --url storofwealth=${URL} ../storofwealth/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit storofwealth=${COMMIT} --url storofwealth=${URL} ../storofwealth/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/storofwealth-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/storofwealth-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/storofwealth-build.assert
```
...
[storofwealth/gitian.sigs](https://github.com/storofwealth-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
